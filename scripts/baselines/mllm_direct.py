"""Unstructured difficulty prompt: the same annotator and task definition, without the rubric.

Isolates what the rubric's structure contributes over the model's raw judgement. Needs the
local llama-server described in README.md; reasoning stays enabled, as for the rubric.

python scripts/baselines/mllm_direct.py --partition calibration --workers 4
"""
import argparse
import base64
...
PROMPT = """Assess how difficult this image is for object detection. Object detection consists of determining the position of the objects in a given image, i.e. generating a rectangular bounding box that tightly frames each detected object and then establishing which of the available categories each one belongs to.

Assign one overall difficulty level from 1 (very easy) to 5 (very difficult).

Return only one natural number: 1, 2, 3, 4, or 5. Do not return an explanation, label, prefix, punctuation, or additional text."""
...
def ask(image_id, image_path):
    ...
    for attempt in range(args.attempts):
        ...
        match = re.fullmatch(r"[^\d]*([1-5])[^\d]*", content)
        if match:
            return image_id, int(match.group(1))
        print(f"No level for {image_id}, attempt {attempt + 1}/{args.attempts}")
