...
def evaluate_with_fiftyone(data, dataset_name, ground_truth_field):
    ...
    try:
        ...
        dataset.evaluate_detections("predictions",gt_field=ground_truth_field,eval_key="mapped_detection",method="coco",iou=0.5,classwise=True,compute_mAP=False,progress=False,)

        for sample in dataset.iter_samples(autosave=True, progress=False):
            set_labels(sample, ground_truth_field, lambda _: "object")
            set_labels(sample, "predictions", lambda _: "object")

        dataset.evaluate_detections("predictions",gt_field=ground_truth_field,eval_key="localization",method="coco",iou=0.5,classwise=True,compute_mAP=False,progress=False,)
